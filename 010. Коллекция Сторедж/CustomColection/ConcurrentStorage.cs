...
namespace CustomColection
{
    public sealed class ConcurrentStorage<T> : ICollection<T>, IEnumerable<T>, IEnumerable, IComparer<ConcurrentStorage<T>> where T : IComparable<T>, IEquatable<T>
    {
        ...
        public ConcurrentStorage(int size, params EventHandler<NotificationEventArgs>?[]? handlers)
        {
        ...
        public ConcurrentStorage(ConcurrentStorage<T>? storage = default) : this(storage?.arr ?? Array.Empty<T>()) { }
        public ConcurrentStorage(IEnumerable<T>? args, params EventHandler<NotificationEventArgs>?[]? handlers)
        {
            id = Interlocked.Increment(ref counter);
            rw = new ReaderWriterLockSlim(LockRecursionPolicy.SupportsRecursion);
            isDisposed = false;
        ...
        public int IndexOf(Func<T, bool>? condition, CancellationToken token = default)
        {
            try
            {
                rw.EnterReadLock();
                IfDisposed();
                if (ptr < 1 || condition is null) return -1;
                for (int i = 0; i < ptr; i++)
                {
                    if (i % pollingCycles == 0) token.ThrowIfCancellationRequested();
        ...
        public int LastIndexOf(Func<T, bool>? condition, CancellationToken token = default)
        {
            try
            {
                rw.EnterReadLock();
        ...
        public void CopyTo(T[] array, int arrayIndex)
        {
            ...
            try
            {
                ...
                if (ptr < 1) return;
                Array.Copy(arr, 0, array, arrayIndex, ptr);
            }
            finally
            {
                rw.ExitReadLock();
        ...
        public bool Remove(Func<T, bool>? condition, CancellationToken token = default)
        {
            try
            {
                rw.EnterReadLock();
        ...
        public bool Contains(Func<T, bool>? condition, CancellationToken token = default)
        {
            try
            {
                rw.EnterReadLock();
        ...
        public void Add(T? item)
        {
            try
            {
                ...
                if (ptr < 1 || (capacity - ptr) < 2) Extend((ptr + 4) * 2);
                arr[ptr] = item;
                ptr++;
        ...
        public void AddHandlers(params EventHandler<NotificationEventArgs>?[]? handlers)
        {
            try
            {
                rw.EnterWriteLock();
        ...
        public void RemoveHandlers(params EventHandler<NotificationEventArgs>?[]? handlers)
        {
            try
            {
                rw.EnterWriteLock();
