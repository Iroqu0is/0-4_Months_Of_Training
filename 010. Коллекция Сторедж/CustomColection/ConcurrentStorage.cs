...
namespace CustomColection
{
    public sealed class ConcurrentStorage<T> : ICollection<T>, IEnumerable<T>, IEnumerable, IComparer<ConcurrentStorage<T>> where T : IComparable<T>, IEquatable<T>
    {
        ...
        public T[] GetFiltered(Func<T, bool>? condition, CancellationToken token = default)
        {
            try
            {
                rw.EnterReadLock();
                IfDisposed();
                if (ptr < 1 || condition is null) return stub;
                var tmp = new ConcurrentStorage<T>(0);
                for (int i = 0; i < ptr; i++)
                {
                    if (i % pollingCycles == 0) token.ThrowIfCancellationRequested();
                    if (condition(arr[i])) tmp.Add(arr[i]);
                }
                if (tmp.Count == 0) return stub;
                tmp.Trim();
                return tmp.arr;
            }
            finally
        ...
        public bool AnyOf(Func<T, bool>? condition, CancellationToken token = default)
        {
            try
            {
                rw.EnterReadLock();
