...
{
    public class Storage<T> : ICloneable, ICollection<T>, IEquatable<Storage<T>>, IComparable<Storage<T>> where T : IComparable<T>, IEquatable<T>
    {
        public event EventHandler<ServiceMessage>? Notification;
        ...
        public Storage(IEnumerable<T>? args)
        {
            stub = new object();
            id = Interlocked.Increment(ref counter);
            if (args is null)
        ...
        public void Clear()
        {
            lock (stub)
            {
                ...
                extentionsCounter = 0;
                Notification?.Invoke(this, new ServiceMessage("Cleared."));
            }
        }
        ...
        public void Add(T? item)
        {
            lock (stub)
            {
                Notification?.Invoke(this, new ServiceMessage("New record added."));
        ...
        {
            var temporary = this.Snapshot().FreezeArr;
            for (int i = 0; i < temporary.Length; i++)
            {
                yield return temporary[i];
            }
        }
    }
