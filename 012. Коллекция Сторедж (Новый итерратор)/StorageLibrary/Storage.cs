...
{
    public sealed class Storage<T> : ICloneable, ICollection<T>, IEquatable<Storage<T>>, IComparable<Storage<T>>, IEnumerator<T>, IEnumerator where T : IComparable<T>, IEquatable<T>
    {
        ...
        // Блок итерратора
        // Здесь есть коварная ошибка, но я пока не понял где
        object IEnumerator.Current { get { return arr[eptr]; } }
        public T Current { get { return arr[eptr]; } }
